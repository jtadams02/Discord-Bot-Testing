...
async def on_message(message):
    ...
    if message.content.startswith("$swear"):
        ...
        swear = msg.split(' ')[1]
        if swear.isspace():
            await message.channel.send("We're sorry, but we cannot add an empty space to the list")
            return
        temp = f # Temp variable I do not think is needed!
        if swear.lower() in bad_words:
            await message.channel.send("We're sorry, but that swear is already on the list!",delete_after=5.0)
            return
        else:
            temp.write('\n'+swear.lower())
            bad_words.append(swear.lower())
            toSend = (swear+' has been added to the swearlist. Thank you for making our server a safer place!')
            await message.channel.send(toSend,delete_after=5.0)
            return
    ...
    if any(word in message.content.lower() for word in bad_words):
            if message.author.id == 743721232326852628:
                await message.channel.send(random.choice(in_the_clear),delete_after=5.0)
                return
            else:
                await message.channel.send(random.choice(bad_responses),delete_after=5.0)
                return
    # Joe Code
